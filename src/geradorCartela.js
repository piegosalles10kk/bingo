...
// Array para armazenar uma cartela gerada
let cartela = [];

// Função para gerar uma nova cartela de Bingo
function gerarCartela() {
    cartela = [];  // Reinicia a cartela a cada nova geração
    for (let i = 0; i < bingo.length; i++) {
        gerarColunaCartela(bingo[i]);  // Gera uma coluna da cartela e a adiciona à cartela
    }
}

// Função para gerar uma coluna de 5 números únicos retirados do array dado
function gerarColunaCartela(array) { 
    let coluna = [];
    for (let i = 0; i < 5; i++) {
        let numeroSorteado;
        do {
